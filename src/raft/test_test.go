...
func TestBackup2B(t *testing.T) {
	...
	DPrintf("leader: %v, dead server: %v %v %v, alive server: %v %v", leader1, (leader1 + 2) % servers,
		(leader1 + 3) % servers, (leader1 + 4) % servers, (leader1), (leader1 + 1) % servers)
	...
	DPrintf("leader: %v, dead server: %v %v, alive server: %v %v %v", leader1, (leader1 + 1) % servers,
		(leader1 + 0) % servers, (leader1 + 2) % servers, (leader1 + 3) % servers, (leader1 + 4) % servers)
	...
	DPrintf("other server %v is disconnect", other)
	...
	DPrintf("all server closed")
	cfg.connect((leader1 + 0) % servers)
	cfg.connect((leader1 + 1) % servers)
	cfg.connect(other)

	DPrintf("Server back to life %v %v %v", leader1, (leader1 + 1) % servers, other)
